...
model.eval()
model.load_state_dict(torch.load("ckpt/speech.oneway.stft.char.pt", map_location=torch.device('cpu')))
...
h0 = torch.zeros([2, 1, model.n_hidden])
y = model(feq, h0) 
y = torch.argmax(y, 2)
...
for i in y:
    if i==i_pre:
        ...
        i_pre = i
f = open("data/data_thchs30/A2_0.wav.trn", "r", encoding="utf-8")
f.readline()
print("标签：", f.readline().replace(" ", "")) 
print("预测：", "".join(words))
