...
for itrmodel, model in enumerate([model1, model2]):
    ...
    optim = torch.optim.SGD(model.parameters(), lr=eta, weight_decay=0.0)
    x1 = x11[:number] 
    d1 = d11[:number]
    acc1, acc2 = [], []
    ...
    ax.set_title(f"样本数量={number}\n训练集={np.max(acc1)*100:.1f}%\n测试集:{np.max(acc2)*100:.1f}%", x=0.5, y=0.5, va="top", ha="left")
plt.savefig("导出图像/过拟合问题.样本数量.jpg")
plt.savefig("导出图像/过拟合问题.样本数量.svg")
print(x_train.shape)
