...
class Model(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            Conv2d(1, 16, 3, 1, 1), # 16个滤波器，输入1个通道灰度图
            nn.AvgPool2d(2, 2), # 降采样
...
for e in range(n_epoch):
    ...
    torch.save(var_dict, "ckpt/mnist.conv2d.pt")
...
plt.savefig("导出图像/手写数字.api.jpg")
plt.savefig("导出图像/手写数字.api.pdf")
print(x_train.shape)
