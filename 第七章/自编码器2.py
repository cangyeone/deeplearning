    ...
    for e in range(1000):
        for x, d in dataloader:
            ...
            if step % 50 ==0:
                ...
                z = torch.randn([100, 2, 1, 1])
                c, _ = model(x, z)
                c = c.permute(0, 2, 3, 1).detach().cpu().numpy() 
                x = x.permute(0, 2, 3, 1).detach().cpu().numpy() 
                z = z.reshape(100, 2).detach().cpu().numpy() 
                h = h.reshape(100, 2).detach().cpu().numpy() 
                d = d.cpu().numpy()
                ...
                ax.set_title("(c) 表示向量分布")
                ax = fig.add_subplot(gs[1, 1]) 
                for i in range(10):
                    ax.scatter(z[d==i, 0], z[d==i, 1], c="k", marker=f"${i}$", s=50)
                ax.set_title("(d) 随机向量分布")


                plt.savefig("导出图像/ae.mnist.t.png")
                plt.savefig("导出图像/ae.mnist.t.svg")
                plt.close()
                print(e, "绘图完成")
